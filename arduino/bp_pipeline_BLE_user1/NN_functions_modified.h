// This code is modified for our project and is originally based on:
...
// - Extension of the softmax cross-entropy loss with optional class weighting.
// - Add per-class accuracy (i.e., class recall).
// - Reduce the logit dynamic range.
...
// ===== Class weights =====
// index = class label
DATA_TYPE class_weight[OUT_VEC_SIZE] = {
    1.0,   // on      (0)
    1.0    // off     (1)
};
...
DATA_TYPE dLossCalc( unsigned int layerIndx, unsigned int nodeIndx) {
	...
	DATA_TYPE Sum = 0;
	// for the last layer, we use class-weighted computation
	if (layerIndx == numLayers - 1) {	

		int trueLabel = -1;
		for (unsigned int k = 0; k < OUT_VEC_SIZE; k++) {
			if (hat_y[k] == 1.0) {
				trueLabel = k;
				break;
			}
		...
		// Extract corresponding class weight
		DATA_TYPE w = 1.0;
		if (trueLabel >= 0 && trueLabel < OUT_VEC_SIZE) {
			w = class_weight[trueLabel];
		}

		// class-weighted gradient (Equation 17 with class weight)
		Sum = w * (y[nodeIndx] - hat_y[nodeIndx]);
...
void backwardProp() {
    ...
    // tracing each node in the layer.
		...
		// Calculate gradients and update weights immediately
		...
		// Calculate bias gradient and update bias immediately
		L[i].Neu[j].dB = -LEARNING_RATE * L[i].Neu[j].dA;
		L[i].Neu[j].B += L[i].Neu[j].dB;
		L[i].Neu[j].dB = 0.0;
    }
  }
}
...
void printAccuracy()
{
  // Per-class accuracy arrays
  int classCorrect[OUT_VEC_SIZE];
  int classTotal[OUT_VEC_SIZE];
  
  // ========== Training Data ==========
  int correctCount = 0;
  // Initialize per-class counters
  for (unsigned int c = 0; c < OUT_VEC_SIZE; c++) {
    classCorrect[c] = 0;
    classTotal[c] = 0;
  }
  ...
  for (unsigned int i = 0; i < numTrainData; i++) {
    ...
    int trueLabel = train_labels[i];
    classTotal[trueLabel]++;
    if (maxIndx == trueLabel) {
      correctCount += 1;
  ...
  Serial.print("Training Per-Class Accuracy: [");
  for (unsigned int c = 0; c < OUT_VEC_SIZE; c++) {
    float classAcc = (classTotal[c] > 0) ? (classCorrect[c] * 1.0 / classTotal[c]) : 0.0;
    Serial.print("Class ");
    Serial.print(c);
    Serial.print(": ");
    Serial.print(classAcc, 3);
    Serial.print(" (");
    Serial.print(classCorrect[c]);
    Serial.print("/");
    Serial.print(classTotal[c]);
    Serial.print(")");
    if (c < OUT_VEC_SIZE - 1) Serial.print(", ");
  }
  Serial.println("]");
  ...
  for (unsigned int i = 0; i < numValData; i++) {
    ...
    int trueLabel = validation_labels[i];
    classTotal[trueLabel]++;
    if (maxIndx == trueLabel) {
      correctCount += 1;
      classCorrect[trueLabel]++;
  ...
  Serial.print("Validation Per-Class Accuracy: [");
  for (unsigned int c = 0; c < OUT_VEC_SIZE; c++) {
    float classAcc = (classTotal[c] > 0) ? (classCorrect[c] * 1.0 / classTotal[c]) : 0.0;
    Serial.print("Class ");
    Serial.print(c);
  ...
  // Print per-class accuracy for test
  Serial.print("Test Per-Class Accuracy: [");
  for (unsigned int c = 0; c < OUT_VEC_SIZE; c++) {
    float classAcc = (classTotal[c] > 0) ? (classCorrect[c] * 1.0 / classTotal[c]) : 0.0;
    Serial.print("Class ");
    Serial.print(c);
