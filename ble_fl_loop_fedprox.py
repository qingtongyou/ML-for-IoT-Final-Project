...
# ===== FedProx-like config =====
USE_FEDPROX_LIKE = True
BETA = 0.2          # proximal strength (0 < beta <= 1)
...
# ===== Global model cache (FedProx-like core) =====
...
def write_weights_to_h(weights: np.ndarray, name: str, out_dir=f"ble_weights_{ratio_user2}_fedprox"):
...
# =========================================================
# FL pairing scheduler (FedProx-like implemented here)
# =========================================================
...
class FLState:
    ...
    def add(self, user_id: str, weights: np.ndarray, session_id: int):
        ...
        if self.queues is None:
            print("⚠️ FLState queues not initialized")
            return

        self.queues[user_id].put_nowait((session_id, weights))
        print(f"🧩 [{user_id}] Weights added to queue (session={session_id}), "
              f"queue length={self.queues[user_id].qsize()}")

    async def try_aggregate(self) -> Optional[np.ndarray]:
        """
        if both queues are non-empty:
            -> perform pairing + aggregation
            -> apply FedProx-like global update
            -> return ndarray
        else:
            -> wait
            -> return None
        ...
        try:
            ...
            # ---------- FedAvg ----------
            w_avg = (1 - ratio_user2) * w0 + ratio_user2 * w1

            # ---------- FedProx-like global update ----------
            if global_model is None:
                global_model = w_avg.copy()
                print("🌱 Initialized global model")
            else:
                if USE_FEDPROX_LIKE:
                    global_model = global_model + BETA * (w_avg - global_model)
                    print(f"🧲 FedProx-like update applied (beta={BETA})")
                else:
                    global_model = w_avg
...
def pack_weights(weights: np.ndarray, session_id: int = 1) -> List[bytes]:
    ...
    print(f"📦 Packing weights: total_size={total_size} bytes, chunks={total_chunks}")
...
async def broadcast_global(global_w: np.ndarray):
    ...
    if broadcasting:
        print("⚠️ Transmission in progress, skipping this request")
        return
    ...
    try:
        print(f"\n🌍 ===== Round {round_id}: Broadcasting Global Weights =====")
...
async def push_to_client(user_id: str, client: BleakClient, packets: List[bytes]):
    ...
    """
    try:
        print(f"📤 [{user_id}] Start sending global weights back to device...")
        ...
        print(f"✅ [{user_id}] Global weights written successfully")

    except Exception as e:
        print(f"⚠️ [{user_id}] Failed to write global weights: {e}")
...
class BLEAggregator:
    ...
    def handle_packet(self, data: bytes):
        ...
        if session_id not in self.sessions:
            print(f"\n🆕 [{self.user_id}] Session {session_id}")
        ...
        if session.is_complete():
            print(f"✅ [{self.user_id}] Session {session_id} COMPLETE")
    ...
    async def on_complete(self, session: WeightSession):
        ...
        print(f"🔢 [{self.user_id}] weights shape = {weights.shape}")
        ...
        if broadcast_queue is not None:
            try:
                ...
                if global_w is not None:
                    print("\n🌍 Running Federated Averaging with FedProx-like update")
                    write_weights_to_h(global_w, "global_fedavg")
                    print("🌍 FedAvg + FedProx DONE")

                    # Trigger broadcast of global model
                    broadcast_queue.put_nowait(global_w)
                else:
                    print(f"⏳ [{self.user_id}] Waiting for the other user's weights...")
            except Exception as e:
                print(f"⚠️ Aggregation error: {e}")
        else:
            print("⚠️ Broadcast queue not initialized")
...
async def resolve_identifier(identifier: str) -> str:
    ...
    print(f"🔍 Scanning for device name: {identifier}")
    devices = await BleakScanner.discover(timeout=5.0)
    for d in devices:
        if d.name == identifier:
            print(f"✅ Found device {identifier}: {d.address}")
            return d.address
...
async def connect_device(user_id: str, identifier: str):
    ...
    while True:
        try:
            ...
            print(f"🔗 [{user_id}] Connecting to {identifier} ({addr})")

            async with BleakClient(addr) as client:
                clients[user_id] = client
                print(f"✅ [{user_id}] successfully connected!")
                ...
                print(f"📡 [{user_id}] Listening for weight updates...")
            ...
            print(f"⚠️ [{user_id}] connection error: {e}")
            if user_id in clients:
                del clients[user_id]

        print(f"🔄 [{user_id}] Reconnecting in 5 seconds...")
...
async def main():
    ...
    print("=" * 60)
    print("BLE-based FL loop (FedProx-like)")
    print("=" * 60)
    print(f"Target devices: {list(DEVICES.keys())}")
    print(f"FedProx-like: {USE_FEDPROX_LIKE}, Beta: {BETA}")
    print(f"User weight ratio: user1={1-ratio_user2}, user2={ratio_user2}")
    print("=" * 60)
    print()
